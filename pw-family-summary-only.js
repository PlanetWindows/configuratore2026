(function(){
  ...
  function val(card,sel){
    ...
    if(!el) return '';
    try{return typeof window.valoreSelect==='function'?(window.valoreSelect(el)||el.value||''):(el.value||'')}catch(_){return el.value||''}
  }
  ...
  function matchRecord(value,card){
    if(!card) return null;
  ...
  function imageFor(value,card){
    const rec=matchRecord(value,card);
    const src=rec?.immagine||'';
    if(!src) return '';
  ...
  function init(){
    patchOpeningImage();
    document.querySelectorAll('.serramento').forEach(storeOnCard);
  }
  ...
  if(document.readyState==='loading'){
    document.addEventListener('DOMContentLoaded',()=>{init();obs.observe(document.body,{childList:true,subtree:true})},{once:true});
  }else{
    init();obs.observe(document.body,{childList:true,subtree:true});
  }
  setTimeout(init,400);
  setTimeout(init,1200);
  window.PW_FAMILY_SUMMARY_VERSION=VERSION;
})();
